...
import com.example.demo.together.boardVolunteer.service.BoardVolunteerService;
...
public class BoardVolunteerController {
    private static final Logger log = LoggerFactory.getLogger(BoardVolunteerController.class);
    @Autowired
    private BoardVolunteerService boardService;

    //게시글 작성. 작성 시 이미지를 temp -> test 폴더로 이동, 글 내용의 img 태그의 src를 갱신한다
    @Secured("ROLE_USER")
    @PostMapping(path = {"writeBoard"})
    public void writeBoard(@RequestBody BoardVolunteerDTO boardDTO) {
    ...
    public Page<BoardVolunteerDTO> getWriteList(@PageableDefault(page = 0,size = 5,sort = {"seq"},direction = Sort.Direction.DESC) Pageable pageable) {
        //System.out.println("글 목록 출력 컨트롤러");
        //페이지 정보를 포함하여 게시글 정보를 받아온다.
        Page<BoardVolunteerDTO> list = this.boardService.getWriteList(pageable);
        return list;
    }
